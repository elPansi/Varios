...
emailRecepcion = [ 'email@envio.com', 'email@recepcion.com' ] #['fernandohc@gmail.com'] #
...
mail['Subject'] = '07.SII. Registro de Facturas Emitidas v1.1'
...
cadArchivo = rutaInformes + cadTiempo + str(mes).zfill(2) + "_factEmitidasPresentadas_v_1.1.xls"

logger = logging.getLogger('zeep')
logger.setLevel(logging.DEBUG)
fh = logging.FileHandler(rutaLog + cadTiempo + str(mes).zfill(2) + '_consulta_FE_v_1.1.log')
...
if salida['RegistroRespuestaConsultaLRFacturasEmitidas']:
    dPeriodo = salida['PeriodoLiquidacion']
...
if sinSalida == 0: #Procesamos la info.
    ...
    for r in resultado:
        ...
        desgFact = tipoDesg['DesgloseTipoOperacion']
        #f.write("Desglose FACTURA: " + str (desgFact) + "\r\n")
        ...
        if desgFact:
            #f.write("desgFact: " + str (desgFact) + "\r\n")
            entrega = desgFact['Entrega']
            sujeta = entrega['Sujeta']
            noSujeta = entrega['NoSujeta']
            if sujeta:
                #f.write("SUJETAA: " + str (sujeta) + "\r\n")
                exenta = sujeta['Exenta']
                noExenta = sujeta['NoExenta']
            if noExenta: #antes ponia noExenta
                tipoNoExenta = noExenta['TipoNoExenta']
                desIVA = noExenta['DesgloseIVA']
                #f.write("Desglose IVA: " + str (desIVA) + "\r\n")
        ...
        if exenta:
            ...
            detExenta = exenta['DetalleExenta']
            base = detExenta[0]['BaseImponible']
        ...
        #Detalle de IVA
            ...
            logger.info("---> Desglose IVA:" + str(desIVA) )
            if desIVA:
