...
mail['Subject'] = '08.SII. Registro de Facturas Recibidas v1.1'
...
exentaIVA = 0
sinSalida = 1
mes = raw_input("""Numero mes (año 2019): """)
anio = 2020

cadTiempo = time.strftime("%Y") + time.strftime("%m") + time.strftime("%d") + time.strftime("%H") + time.strftime("%M") + time.strftime("%S") + "_"
cadArchivo = rutaInformes + cadTiempo + str(mes).zfill(2) + "_factRecibidasPresentadas_v_1.1.xls"

logger = logging.getLogger('zeep')
logger.setLevel(logging.DEBUG)
fh = logging.FileHandler(rutaLog + cadTiempo + str(mes).zfill(2) + '_consulta_FR_v_1.1.log')
