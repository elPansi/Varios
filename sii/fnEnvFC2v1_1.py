...
def enviaFacturaCompra(serieFactura, numeroFactura):
    ...
    sqlRecuperaImpuesto = """SELECT FC.CODFACTURACOMPRA, FS.SERIE, FC.NumDocPropio, FL.IVA,
    ...
    FROM FACTURACABCOMPRA FC FULL OUTER JOIN articulosdetallesCompra FL ON FC.CODFACTURACOMPRA = FL.CODFACTURACOMPRA
    INNER JOIN FACTURASERIE FS ON FS.CODSERIE = FC.SERIE
    WHERE FS.Serie = '{SERIEFACTURA}' AND FC.NumDocPropio = {NUMEROFACTURA} AND FL.IVA = {IVA}
    ...
    facturaManual = """{
    	...
    	"IDFactura":{
    		...
    		"NumSerieFacturaEmisor":"1262201",
    		"FechaExpedicionFacturaEmisor":"31/08/2017"
    		},
    	"FacturaRecibida":{
    		"TipoFactura":"F1",

    		"ClaveRegimenEspecialOTrascendencia":"09",
    		"DescripcionOperacion":"COMPRA DE AUTOMATISMOS",
              "DesgloseFactura":{
                                 "DesgloseIVA":{
                                    "DetalleIVA": [
                                            	{
    							"TipoImpositivo":0.00,
    							"BaseImponible":     1203.57,
    							"CuotaSoportada":0.00
    						}
                                     ]
                                 }
    		...
    		"Contraparte":{
    			"NombreRazon":"MARANTEC GMBH & CO. KG",
    			"IDOtro":{
                     "CodigoPais":"DE",
    			     "IDType":"04",
    				 "ID":"DE126734694"
    ...
    fh = logging.FileHandler(rutaLog + cadTiempo + '_compra_logSii_v1.1.log')
